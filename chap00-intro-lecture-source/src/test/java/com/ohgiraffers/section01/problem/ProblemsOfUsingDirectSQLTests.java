...
public class ProblemsOfUsingDirectSQLTests {
    ...
    /* 수업목표. JDBC API를 이용하여 직접 SQL을 작성할 때 발생하는 문제점에 대해 이해할 수 있다. */
    /* 필기.
     *  <JDBC API를 이용해 직접 SQL을 다룰 때 발생하는 문제점>
     *  1. 데이터 변환, SQL작성, JDBC API 코드 등을 중복 작성(개발 시간 증가, 유지보수성 악화)
     *  2. SQL에 의존적인 개발
    ...
    /* 목차. 1. 데이터 변환, SQL작성, JDBC API 코드 등을 중복 작성 */
    ...
    void testDirectSelectSQL() throws SQLException {
        ...
        //when
        /* 설명. DB 연결이 필요한 경우 매번 JDBC API 코드를 반복적으로 사용해야 한다. */
        Statement stmt = con.createStatement();
        ResultSet rset = stmt.executeQuery(query);

        /* 설명. 조회한 데이터를 자바 객체로 개발자가 직접 변환을 해 주어야 한다. */
    ...
    void testDirectInsertSQL() throws SQLException {
        ...
        //when
        /* 설명. insert 역시 반복되는 JDBC API 코드와 데이터 변환 작업등을 개발자가 직접 해 주어야 한다. */
    ...
    void testChangeSelectColumns() throws SQLException {
        ...
        //given
        /* 설명. 최초 요구사항은 메뉴코드와 메뉴이름을 조회하는 것이다. */
...
//            menuAndCategory.setCategory(new Category(rset.getInt("CATEGORY_CODE"), rset.getString("CATEGORY_NAME")));
    ...
    /* 목차. 3. 패러다임 불일치의 문제가 발생할 수 있다. */
    /* 설명.
     *  관계형 데이터 베이스는 데이터 중심으로 구조화 되어 있고, 집합적인 사고를 요구한다.
     *  그리고 객체지향에서 이야기하는 추상화, 상속, 다형성 같은 개념이 존제하지 않는다.
     ...
     *  서브타입 엔터티로 법인과 일반회원이 존재하는 형태인 경우
     *  JDBC API를 이용하면
     *  INSERT INTO 회원 ...
     *  INSERT INTO 법인회원 ...
     *
     ...
     *  Team team = member.getTeam();  <- 연관객체를 함께 조인해서 조회하는 것이 보장된다. (NPE 발생하지 않음)
     * */
