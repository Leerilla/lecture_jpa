...
public class EntityManagerGeneratorTests {
    ...
    /* 수업목표. 영속성 컨텍스트(persistence context)와 엔티티 매니저(entity manager)에 대해 이해할 수 있다.. */
    /* 필기.
     ...
     *   엔티티 매니저는 스레드세이프 하지 않기 때문에 동시성 문제가 발생할 수 있기 때문에 스레드간 공유를 하면 안된다.
     ...
     *   영속성 엔티티를 key value방식으로 저장하는 저장소 역할을 한다.
